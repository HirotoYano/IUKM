...
def main():
    ...
    data_path: str = f"{OUTPUT_PATH}/cos_similarity_to_com_unique_utterance_v2/2023-8-29/6-50-5"
    geleral_interpellation_list: List[str] = os.listdir(data_path)
    current_date, current_time = get_current_datetime()

    for general_interpellation in geleral_interpellation_list:
        data_file_path: str = f"{data_path}/{general_interpellation}"
        file_name_list: List[str] = os.listdir(data_file_path)
        # dim_list: List[int] = [3, 5, 10, 20, 30, 40, 50]
        dim_list: List[int] = [10]

        save_path: str = f"{OUTPUT_PATH}/clause_filtering_v2/{current_date}/{current_time}/{general_interpellation}"
        make_dir(save_path)

        for i, file_name in enumerate(tqdm(file_name_list)):
            df_cos_sim_com: pl.DataFrame = data_loder(
                file_path=f"{data_path}/{general_interpellation}/{file_name}", has_header=True
            )
