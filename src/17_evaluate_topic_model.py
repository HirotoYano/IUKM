...
def main():
    ...
    for general_interpellation in general_interpellation_list:
        digest_data_path: str = f"{INTERIM_DATA_PATH}/speaker_utterance_dataset/{general_interpellation}/digest.csv"
        ...
        tf_idf_result_data_path: str = f"{OUTPUT_PATH}/tf_idf_after_filtering_v2/2023-8-29/19-38-2/一般質問(要旨)12月9日/top_five_words/top_five_words_10_dimension.csv"
        df_tf_idf_result: pl.DataFrame = data_loder(file_path=tf_idf_result_data_path, has_header=True)

        topic_model_result_data_path: str = f"{OUTPUT_PATH}/topic_model/2023-8-29/19-56-28/{general_interpellation}.csv"
            ...
            for short_summary in short_summary_list:
                for cluster in cluster_list:
                    if df_tf_idf_result[cluster][i] in short_summary:
                        print(short_summary)
                        print(df_tf_idf_result[cluster].to_list()[0:5])
        ...
        cluster_list: List[str] = df_topic_model_result.columns
        shuffle(cluster_list)
            ...
            for short_summary in short_summary_list_topic_model:
                for cluster in cluster_list:
                    if df_topic_model_result[cluster][i] in short_summary:
                        print(short_summary)
                        print(df_topic_model_result[cluster].to_list()[0:5])
        ...
        print(f"{general_interpellation}")
