...
def main():
    ...
    assembly_cluster_path: str = f"{OUTPUT_PATH}/dendrogram/2023-6-11/14-48-45/assembly_cluster.csv"
    df_assembly_add_cluster: pl.DataFrame = data_loder(assembly_cluster_path, has_header=True)
    ...
    for cluster in clusters:
        ...
        for i, vector_per_cluster in enumerate(vectors_per_cluster):
            ...
            df: pl.DataFrame = df_assembly_add_cluster_filter[i].with_columns(new_seires)
    ...
    save_csv(
        ...
        file_name="cos_similarity_between_unique_utterance_vector_and_com.csv",  # フィルタリング用
    )
